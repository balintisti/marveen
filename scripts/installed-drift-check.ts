...
type Finding = { name: string; lane: string; text: string; drift: boolean };
...
function record(lane: string, name: string, d: Direction, sourceRel: string): void {
  const { text, drift } = verdict(d, sourceRel);
  findings.push({ lane, name, text, drift });
}
  ...
  try {
    ...
    for (const inst of SAFE_INSTALLERS) {
      ...
      for (const g of walk(genDir).sort()) {
        ...
        if (sameAsInstaller(norm, live)) { findings.push({ lane: 'D hook', name: rel, text: 'AZONOS', drift: false }); continue; }
        record('D hook', rel, classifyDirection(norm, live), `scripts/${inst}`);
      }
    }
  } finally {
...
function main(): number {
  ...
  for (const f of findings.filter((x) => !x.drift)) console.log(`  ${f.lane.padEnd(8)} ${f.name.padEnd(46)} ${f.text}`);
  if (!drifting.length) {
