#!/bin/bash
# A harom kapacitas-szam es a tartos-szabad-keret verdikt JSON-ban -- a napindito EGY parancsa (kartya 54ee459b).
#
...
#
# Usage: scripts/capacity-report.sh [--hours N] [--calendar ID]
set -uo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CLI="$ROOT/dist/capacity-cli.js"
...
if [ -f "$CLI" ]; then
  VIA="dist"
  CMD=("$NODE" "$CLI")
elif [ -f "$ROOT/src/capacity-cli.ts" ] && [ -x "$TSX" ]; then
  VIA="tsx-source (nincs build -- 'npm run build' utan a dist szolgal ki)"
  CMD=("$TSX" "$ROOT/src/capacity-cli.ts")
elif [ -f "$ROOT/src/capacity-cli.ts" ]; then
  emit_error "dist/capacity-cli.js hianyzik, a forras megvan, de tsx sincs -- 'npm run build' kell"
else
  emit_error "dist/capacity-cli.js hianyzik (es a forras sem talalhato: $ROOT/src/capacity-cli.ts)"
fi

ERR_FILE="$(mktemp -t capacity-report)"
OUT="$(CAPACITY_VIA="$VIA" "${CMD[@]}" "$@" 2>"$ERR_FILE")"
RC=$?
STDERR="$(tr '\n' ' ' < "$ERR_FILE" | cut -c1-200)"
rm -f "$ERR_FILE"
