...
# THE BODY IS A FUNCTION BEHIND A GUARD, AND HERE THE SIDE EFFECT IS DESTRUCTIVE
# (card e479b940). This module rewrites its target IN PLACE -- `open(SRC, 'w')`
# twice, once to plant a mutation and once to restore -- and SRC defaults to a
# TRACKED file, src/web/schedule-runner.ts. Unguarded, an importer carrying two
# argv entries of its own would start a full sweep over somebody else's working
# tree: npx vitest runs, source rewritten, and the restore sits in a `finally`
# that a hard crash can still skip.
#
# The bare import was already loud (IndexError on sys.argv[1], before any write).
# The guard is for the case that is not: argv inherited from the importer.
